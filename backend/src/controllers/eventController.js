...
const getAll = async (req, res) => {
  try {
    ...
    // Exclude BLOB columns from list query for performance
    const [rows] = await pool.query(
      `SELECT e.id, e.title, e.slug, e.banner, e.description, e.category_id, e.organizer_id,
              e.location, e.maps_link, e.start_date, e.end_date, e.registration_deadline,
              e.max_quota, e.visibility, e.status, e.is_started, e.created_at,
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT e.id, e.title, e.slug, e.banner, e.description, e.category_id, e.organizer_id,
              ...
              c.name as category_name, c.slug as category_slug,
              o.name as organizer_name, o.logo as organizer_logo,
              o.email as organizer_email, o.phone as organizer_phone,
              (SELECT COUNT(*) FROM registrations r WHERE r.event_id = e.id AND r.status = 'registered') as participants
       FROM events e
       LEFT JOIN categories c ON e.category_id = c.id
       LEFT JOIN organizers o ON e.organizer_id = o.id
    ...
    // Access control: private events only for authenticated users
    if (event.visibility === 'private' && !req.user) {
      return res.status(403).json({ message: 'Acara ini hanya untuk pengguna internal.' });
    }

    // Speakers
    const [speakers] = await pool.query(
      `SELECT s.id, s.name, s.photo, s.position, s.institution, s.biography FROM speakers s
    ...
    const [gallery] = await pool.query(
      'SELECT id, event_id, image_path, created_at FROM galleries WHERE event_id = ?',
      [id]
    );
...
const create = async (req, res) => {
  try {
    ...
    // Banner disimpan sebagai BLOB di DB
    const bannerData = req.file ? req.file.buffer   : null;
    const bannerType = req.file ? req.file.mimetype : null;

    const [result] = await pool.query(
      `INSERT INTO events (title, slug, banner_data, banner_type, description, category_id, organizer_id, location, maps_link,
        start_date, end_date, registration_deadline, max_quota, visibility, status)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [title.trim(), slug, bannerData, bannerType, description || null,
    ...
    // Set banner URL ke endpoint serve gambar
    const bannerUrl = bannerData ? `/api/images/events/${eventId}` : null;
    if (bannerUrl) {
      await pool.query('UPDATE events SET banner = ? WHERE id = ?', [bannerUrl, eventId]);
    }
...
const update = async (req, res) => {
  try {
    ...
    if (req.file) {
      // Simpan banner baru ke DB sebagai BLOB
      fields.push('banner_data = ?'); values.push(req.file.buffer);
      fields.push('banner_type = ?'); values.push(req.file.mimetype);
      fields.push('banner = ?');      values.push(`/api/images/events/${id}`);
    }
...
const remove = async (req, res) => {
  try {
    ...
    if (!rows.length) return res.status(404).json({ message: 'Acara tidak ditemukan.' });
    // Cascade delete: event_speakers, galleries, registrations will be deleted by FK ON DELETE CASCADE
...
// ─────────────────────────────────────────────────────────────────────────────
// PUT /api/events/:id/attendance/:registrationId
// ─────────────────────────────────────────────────────────────────────────────
