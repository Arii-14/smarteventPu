...
const getAll = async (req, res) => {
  try {
    ...
    if (err.message.includes('does not exist') || err.message.includes("doesn't exist")) {
      return res.status(500).json({ message: 'Tabel about_developers belum ada. Jalankan inisialisasi database.' });
    }
...
const create = async (req, res) => {
  try {
    const { name, role_title, description, github_url, instagram_url, facebook_url, whatsapp_number, display_order } = req.body;
    ...
    const [result] = await pool.query(
      `INSERT INTO about_developers (name, role_title, description, photo_data, photo_type, github_url, instagram_url, facebook_url, whatsapp_number, display_order)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [name.trim(), role_title || null, description || null, photoData, photoType, github_url || null, instagram_url || null, facebook_url || null, whatsapp_number || null, display_order || 0]
    );
...
const update = async (req, res) => {
  try {
    ...
    return res.status(500).json({ message: 'Gagal memperbarui data pengembang.' });
  }
};
...
const remove = async (req, res) => {
  try {
    ...
    console.error('[ABOUT] remove error:', err.message);
    return res.status(500).json({ message: 'Gagal menghapus data pengembang.' });
  }
};
