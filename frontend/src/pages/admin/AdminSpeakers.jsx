...
const AdminSpeakers = () => {
  ...
  const openModal = (sp = null) => {
    if (sp) {
      ...
      setPhotoPreview(sp.photo ? `${import.meta.env.PROD ? '' : 'http://localhost:5000'}${sp.photo}` : null);
    } else {
      setEditingId(null);
            ...
            >
              ...
              <div className="flex flex-col items-center text-center pt-2">
                <div className="relative mb-3">
                  <img
                    src={sp.photo ? `${import.meta.env.PROD ? '' : 'http://localhost:5000'}${sp.photo}` : `https://ui-avatars.com/api/?name=${encodeURIComponent(sp.name)}&background=7c3aed&color=fff&size=80&bold=true`}
                    alt={sp.name}
