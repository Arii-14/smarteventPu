            ...
            {favorites.map((event, index) => (
              ...
              >
                <div className="relative aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-800">
                  <img
                    src={event.banner ? (event.banner.startsWith('http') || event.banner.startsWith('data:') ? event.banner : `${import.meta.env.PROD ? '' : 'http://localhost:5000'}${event.banner.startsWith('/') ? '' : '/'}${event.banner}`) : `https://source.unsplash.com/random/800x600?event,${event.id}`}
                    alt={event.title}
