    ...
    <div className="bg-slate-50 dark:bg-slate-900 min-h-screen pb-20">
      ...
      <div className="relative h-[40vh] md:h-[50vh] w-full group">
        <div className="absolute inset-0 bg-slate-900 overflow-hidden">
          <img
            src={event.banner ? (event.banner.startsWith('http') || event.banner.startsWith('data:') ? event.banner : `http://localhost:5000${event.banner.startsWith('/') ? '' : '/'}${event.banner}`) : `https://images.unsplash.com/photo-1540575467063-178a50c2df87?auto=format&fit=crop&q=80&w=1600`}
            alt={event.title}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {speakers.map(spk => (
                    <div key={spk.id} className="flex items-center gap-4 p-4 rounded-xl bg-slate-50 dark:bg-slate-700/40 border border-slate-200/60 dark:border-slate-600/40 hover:border-indigo-300 dark:hover:border-indigo-700 transition-colors group">
                      <img
                        src={spk.photo ? (spk.photo.startsWith('http') || spk.photo.startsWith('data:') ? spk.photo : `http://localhost:5000${spk.photo.startsWith('/') ? '' : '/'}${spk.photo}`) : `https://ui-avatars.com/api/?name=${encodeURIComponent(spk.name)}&background=6366f1&color=fff&size=80&bold=true`}
                        alt={spk.name}
