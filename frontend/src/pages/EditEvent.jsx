...
import {
  ...
  Trash2, CheckCircle, ChevronDown, Search, Loader2
} from 'lucide-react';
...
const EditEvent = () => {
  ...
  useEffect(() => {
    const loadAll = async () => {
      try {
        const [catRes, orgRes, spkRes, eventRes] = await Promise.all([
        ...
        setFormData({
          ...
          start_date: toLocal(ev.start_date),
          end_date: toLocal(ev.end_date),
          registration_deadline: toLocal(ev.registration_deadline),
          max_quota: ev.max_quota || '',
          visibility: ev.visibility || 'public',
          status: ev.status || 'draft',
        });

        if (ev.banner) {
          const bannerStr = String(ev.banner);
          const bannerUrl = bannerStr.startsWith('http') ? bannerStr : `${import.meta.env.PROD ? '' : 'http://localhost:5000'}${bannerStr}`;
          setPreviewUrl(bannerUrl);
        }

        if (ev.speakers && Array.isArray(ev.speakers)) {
          setSelectedSpeakers(ev.speakers.map(s => ({ id: s.id, name: s.name, position: s.position, institution: s.institution, photo: s.photo })));
        }
      } catch (err) {
  ...
  const processFile = (f) => { setFile(f); setPreviewUrl(URL.createObjectURL(f)); };
  const handleFileChange = (e) => { if (e.target.files?.[0]) processFile(e.target.files[0]); };
  const handleChange = (e) => { setFormError(''); setFormData(prev => ({ ...prev, [e.target.name]: e.target.value })); };

  const toggleSpeaker = (spk) => {
    setSelectedSpeakers(prev =>
      prev.find(s => s.id === spk.id) ? prev.filter(s => s.id !== spk.id) : [...prev, spk]
    );
  };

  const handleCreateSpeaker = async () => {
    if (!newSpeaker.name) { toast.error('Nama pembicara wajib diisi'); return; }
    ...
    try {
      ...
      const created = { id: res.data.speaker_id || res.data.id, name: newSpeaker.name, position: newSpeaker.position, institution: newSpeaker.institution, photo: res.data.photo };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.status === 'published') {
      if (!formData.location.trim()) { setFormError('Lokasi wajib diisi untuk acara yang diterbitkan.'); return window.scrollTo({ top: 0, behavior: 'smooth' }); }
      if (!formData.start_date) { setFormError('Tanggal mulai wajib diisi untuk acara yang diterbitkan.'); return window.scrollTo({ top: 0, behavior: 'smooth' }); }
    }
    if (formData.start_date && formData.end_date) {
      if (new Date(formData.end_date) <= new Date(formData.start_date)) { setFormError('Tanggal berakhir harus setelah tanggal mulai.'); return window.scrollTo({ top: 0, behavior: 'smooth' }); }
    }
    if (formData.registration_deadline && formData.start_date) {
      if (new Date(formData.registration_deadline) > new Date(formData.start_date)) { setFormError('Batas pendaftaran harus sebelum atau sama dengan tanggal mulai acara.'); return window.scrollTo({ top: 0, behavior: 'smooth' }); }
    }
    ...
    try {
      ...
      Object.entries(formData).forEach(([k, v]) => {
        if (k === 'max_quota') {
          if (formData.visibility !== 'public' && v) payload.append(k, v);
        } else if (v !== '') {
          payload.append(k, v);
        }
      });
  ...
  const filteredSpeakers = speakers.filter(s => s.name.toLowerCase().includes(speakerSearch.toLowerCase()));

  if (fetching) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        <Loader2 size={32} className="text-indigo-500 animate-spin" />
        <p className="text-sm text-slate-500 dark:text-slate-400">Memuat data acara...</p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2 text-xs font-semibold text-slate-500 dark:text-slate-400">
            ...
            <span>/</span>
            <Link to="/admin/events" className="hover:text-indigo-500 transition-colors">Acara</Link>
            <span>/</span>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1 text-sm">Perbarui detail acara "{formData.title}".</p>
        </div>
        <Link to="/admin/events" className="px-4 py-2 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-700 transition-colors whitespace-nowrap">
          ← Kembali
        </Link>
      </div>
      ...
      {formError && (
        <div className="bg-rose-50 dark:bg-rose-900/20 border-l-4 border-rose-500 p-4 rounded-r-xl animate-fade-in-up">
          <div className="flex">
            <div className="flex-shrink-0"><X className="h-5 w-5 text-rose-500" /></div>
            <div className="ml-3"><p className="text-sm text-rose-700 dark:text-rose-400 font-semibold">{formError}</p></div>
          </div>
        </div>
      )}
      ...
      <FormSection title="Spanduk / Banner Acara" icon={ImageIcon}>
        <div
          ...
          className={`relative rounded-2xl overflow-hidden border-2 border-dashed transition-all duration-300 ${
            dragActive ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/10 scale-[1.01]' : 'border-slate-300 dark:border-slate-700 hover:border-indigo-400 dark:hover:border-indigo-600 hover:bg-slate-50 dark:hover:bg-slate-800/50'
          }`}
        >
          ...
          {previewUrl ? (
            <div className="relative">
              ...
              <button type="button" onClick={(e) => { e.stopPropagation(); setFile(null); setPreviewUrl(null); }} className="absolute top-3 right-3 w-8 h-8 rounded-full bg-rose-500 text-white flex items-center justify-center hover:bg-rose-600 transition-colors shadow-lg">
                <X size={14} />
              </button>
            </div>
          ) : (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            <InputField label="Judul Acara" required>
              <input name="title" value={formData.title} onChange={handleChange} required type="text" placeholder="Contoh: Seminar Nasional AI 2026" className={inputClass} />
            </InputField>
          </div>
          ...
          <InputField label="Visibilitas">
            <div className="grid grid-cols-2 gap-2">
              {['public', 'private'].map((v) => (
                <button key={v} type="button" onClick={() => setFormData(prev => ({ ...prev, visibility: v }))}
                  className={`flex items-center justify-center gap-2 py-2.5 rounded-xl border-2 text-sm font-semibold transition-all ${
                    formData.visibility === v ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400' : 'border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-slate-300'
                  }`}>
          ...
          <InputField label="Kuota Peserta">
            <div className="relative">
              <Users size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
              <input name="max_quota" value={formData.visibility === 'public' ? '' : formData.max_quota} onChange={handleChange} type="number" min="0" placeholder={formData.visibility === 'public' ? 'Tanpa Batas' : 'Contoh: 200'} disabled={formData.visibility === 'public'} className={`${inputClass} pl-10 ${formData.visibility === 'public' ? 'opacity-50 cursor-not-allowed bg-slate-100 dark:bg-slate-800' : ''}`} />
            </div>
          </InputField>
          ...
          <InputField label="Lokasi" required>
            <div className="relative">
              ...
              <input name="location" value={formData.location} onChange={handleChange} type="text" placeholder="Contoh: Auditorium Utama" className={inputClass + ' pl-10'} />
            </div>
          </InputField>

          <InputField label="Tautan Maps">
            <div className="relative">
              <LinkIcon size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
              <input name="maps_link" value={formData.maps_link} onChange={handleChange} type="url" placeholder="https://maps.google.com/..." className={inputClass + ' pl-10'} />
            </div>
          </InputField>
          ...
          <div className="md:col-span-2">
            <InputField label="Deskripsi Acara">
              <textarea name="description" value={formData.description} onChange={handleChange} rows={5} placeholder="Tuliskan deskripsi lengkap acara..." className={inputClass + ' resize-none'} />
            </InputField>
          </div>
        </div>
      </FormSection>
      ...
      <FormSection title="Organisasi Penyelenggara" icon={Building2}>
        <InputField label="Pilih Penyelenggara">
          <div className="relative">
        ...
        {formData.organizer_id && organizers.find(o => o.id == formData.organizer_id) && (
          <div className="flex items-center gap-3 p-3.5 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl border border-indigo-200 dark:border-indigo-800">
            ...
            <div>
              <p className="text-sm font-bold text-indigo-700 dark:text-indigo-400">{organizers.find(o => o.id == formData.organizer_id)?.name}</p>
              <p className="text-xs text-indigo-600/70 dark:text-indigo-500">{organizers.find(o => o.id == formData.organizer_id)?.email || 'Tidak ada email'}</p>
            </div>
          </div>
        )}
      </FormSection>
      ...
      <FormSection title="Pembicara Acara" icon={Mic2}>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {selectedSpeakers.map(spk => (
                <span key={spk.id} className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400 text-sm rounded-full border border-indigo-200 dark:border-indigo-800 font-medium">
                  <span>{spk.name}</span>
                  <button type="button" onClick={() => toggleSpeaker(spk)} className="hover:text-rose-500 transition-colors"><X size={13} /></button>
                </span>
              ))}
            </div>
          </div>
        ...
        <div className="flex items-center gap-2 mb-4">
          <div className="relative flex-1">
            ...
            <input type="text" placeholder="Cari pembicara..." value={speakerSearch} onChange={e => setSpeakerSearch(e.target.value)} className={inputClass + ' pl-10'} />
          </div>
          <button type="button" onClick={() => setShowNewSpeakerForm(!showNewSpeakerForm)} className="px-4 py-2.5 rounded-xl bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 font-semibold text-sm hover:bg-indigo-100 transition-colors whitespace-nowrap">
            + Buat Baru
          </button>
        </div>
        ...
        {speakers.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-64 overflow-y-auto pr-1">
            {filteredSpeakers.map(spk => {
              ...
              return (
                <button key={spk.id} type="button" onClick={() => toggleSpeaker(spk)}
                  className={`flex items-center gap-3 p-3 rounded-xl border-2 text-left transition-all ${isSelected ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-slate-700 hover:border-indigo-300 hover:bg-slate-50'}`}>
                  <img src={spk.photo ? `${import.meta.env.PROD ? '' : 'http://localhost:5000'}${spk.photo.startsWith('/') ? '' : '/'}${spk.photo}` : `https://ui-avatars.com/api/?name=${encodeURIComponent(spk.name)}&background=6366f1&color=fff&size=40`} alt={spk.name} className="w-9 h-9 rounded-full object-cover shrink-0" />
                  <div className="min-w-0">
                    <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{spk.name}</p>
                    <p className="text-xs text-slate-500 truncate">{spk.position || spk.institution || 'Pembicara'}</p>
                  </div>
                  {isSelected && <CheckCircle size={16} className="text-indigo-600 shrink-0 ml-auto" />}
                </button>
              );
            })}
          </div>
        ) : (
          <p className="text-xs text-amber-600 bg-amber-50 dark:bg-amber-900/20 px-3 py-2 rounded-lg border border-amber-200 dark:border-amber-800 mt-3">Belum ada data pembicara di server.</p>
        )}
      </FormSection>

      {/* Action Buttons */}
      <div className="flex flex-col sm:flex-row justify-end gap-3 pb-6">
        <button type="button" onClick={() => navigate('/admin/events')} className="px-6 py-2.5 rounded-xl border-2 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 font-semibold text-sm hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
          Batal
        </button>
        <button type="button" onClick={handleSubmit} disabled={loading}
          ...
          {loading ? (<><div className="w-4 h-4 rounded-full border-2 border-white/30 border-t-white animate-spin" /> Menyimpan...</>) : 'Simpan Perubahan'}
        </button>
      </div>
    </div>
  );
