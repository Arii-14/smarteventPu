          ...
          {latestEvents.map((ev) => (
            <div key={ev.id} className="flex items-center gap-3.5 px-5 py-3.5 hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors group">
              <div className="relative shrink-0 overflow-hidden rounded-xl h-12 w-16 border border-slate-200/70 dark:border-slate-700/70">
                <img
                  src={ev.banner ? (ev.banner.startsWith('http') || ev.banner.startsWith('data:') ? ev.banner : `${import.meta.env.PROD ? '' : 'http://localhost:5000'}${ev.banner.startsWith('/') ? '' : '/'}${ev.banner}`) : 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=100&auto=format'}
                  alt={ev.title}
